...
noDelay water_pump_interval(9000); //15mins
...
void loop() {
  ...
  if (us_cam_detect_interval.update() && cam_us.convert_cm(cam_us.ping_median(5)) <= cam_us_max_distance && busy_camera == false && busy_water_trash == false)
  {
    busy_camera = true;  
    detected();
  }  
  ...
  if (Serial.available())
  {
    ...
    {
      if (command.equals(reject[i])||command.equals("reject")||command.equals("no object detected"))
      {
        led_busy();
        pltf_reject();
        busy_camera = false;
...
void led_activate()
{
  ...
  else if(busy_camera == false && busy_water_trash == false)
  {
    led_available();
  }   
  else if(busy_water_trash == true)
    led_busy();  
}
