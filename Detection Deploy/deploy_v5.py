...
if developer_mode:
    custom_model_path = 'detect_custom.tflite'
    custom_label_path = 'label_map_custom.txt'
    custom_min_conf_threshold = 0.5
    custom_box_color = (0, 255, 255)  # Yellow

    coco_model_path = 'detect_coco.tflite'
    coco_label_path = 'label_map_coco.txt'
    coco_min_conf_threshold = 0.5
    coco_box_color = (0, 255, 0)  # Green
    history_dir = "history_developer"
else:
    custom_model_path = 'detect_custom.tflite'
    custom_label_path = 'label_map_custom.txt'
    custom_min_conf_threshold = 0.5
...
ser = serial.Serial('COM5', 9600) #'/dev/ttyUSB0'
ser.flush()
...
def detect_objects(interpreter, image, labels, min_conf_threshold, box_color):
    ...
    input_data = preprocess_image(image, input_details[0]['shape'][2], input_details[0]['shape'][1],
        ...
        if ser.in_waiting > 0:
            ...
            if line == "cam_detected":
                ...
                if developer_mode:
                    ...
                    image = cv2.addWeighted(image_custom, 1, image_coco, 1, 0)
                else:
